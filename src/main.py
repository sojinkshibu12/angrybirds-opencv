...
full_sprite = pygame.image.load(
    "../resources/images/full-sprite.png")
cursor = pygame.image.load("../resources/images/angry-birds-game-hand-pack.png")
rect = pygame.Rect(0,0,325.5,326)
cursor1 = cursor.subsurface(rect)
cursor1 = pygame.transform.scale(cursor1,(50,50))
rect = pygame.Rect(325.5,0,300,300)
cursor2 = cursor.subsurface(rect)
...
BLACK = (0, 0, 0)
WHITE = (255, 255, 255)
sling_x, sling_y = 270, 540
sling2_x, sling2_y = 270,540
score = 0
game_state = 0
bird_path = []
counter = 0
...
static_body.position = (0,-50)
static_lines = [pm.Segment(static_body, (0.0, 00.0), (1920.0, 00.0), 0.0)]
static_lines1 = [pm.Segment(static_body, (1200.0, 1000.0), (1200.0, 1000.0), 0.0)]
...
def draw_level_cleared():
    ...
    if level.number_of_birds >= 0 and len(pigs) == 0:
        ...
        game_state = 4
        rect = pygame.Rect(300, 0, 600, 1080)
        pygame.draw.rect(screen, BLACK, rect)
        screen.blit(level_cleared, (450, 100))
        if score >= level.one_star and score <= level.two_star:
            screen.blit(star1, (310, 250))
        if score >= level.two_star and score <= level.three_star:
            screen.blit(star1, (310, 250))
            screen.blit(star2, (500, 230))
        if score >= level.three_star:
            screen.blit(star1, (310, 250))
            screen.blit(star2, (500, 230))
            screen.blit(star3, (700, 270))
        screen.blit(score_level_cleared, (550, 500))
        screen.blit(replay_button, (510, 600))
        screen.blit(next_button, (620, 600))
...
def draw_level_failed():
    ...
    if level.number_of_birds <= 0 and time.time() - t2 > 5 and len(pigs) > 0:
        ...
        rect = pygame.Rect(300, 0, 600, 1080)
        pygame.draw.rect(screen, BLACK, rect)
        screen.blit(failed, (450, 100))
        screen.blit(pig_happy, (400, 200))
        screen.blit(replay_button, (550, 600))
...
while running:
    ...
    for event in pygame.event.get():
        ...
        if (pygame.mouse.get_pressed()[0] and x_mouse > 200 and
                x_mouse < 300 and y_mouse > 370 and y_mouse < 550):
            mouse_pressed = True
    ...
    rect = pygame.Rect(50, -10, 70, 220)
    screen.blit(sling_image, (250, 520), rect)
    ...
    # Draw birds
    for bird in birds:
        if bird.shape.body.position.y < -205:
            birds_to_remove.append(bird)
    ...
    rect = pygame.Rect(0, -5, 60, 200)
    screen.blit(sling_image, (235, 520), rect)
    # Draw score
    ...
    cv2.rectangle(img,(framer,framer),(cap_w-100,cap_h-100),(225,0,225),2)
    if hands:
        lmlist = hands[0]["lmList"]
        indexx , indexy = lmlist[9][0],lmlist[9][1]
        cv2.circle(img, (indexx,indexy),5,(225,225,225),5)
        newx = int(np.interp(indexx,(framer,cap_w-100),(0,1920)))
        newy = int(np.interp(indexy,(framer,cap_h-100),(0,1080)))
        clocx = plocx + (newx - plocx)/smoothening
        clocy = plocy + (newy - plocy)/smoothening
        fingers = detector.fingersUp(hands[0])
        print(fingers)
        mouse.position = (clocx,clocy)
        if mouse_pressed == True:
            # mouse.press(Button.left)
            screen.blit(cursor2,(clocx,clocy))
        else:
            # mouse.release(Button.left)
            screen.blit(cursor1,(clocx,clocy))
        plocx,plocy = clocx,clocy
    cv2.imshow("cap-screen",img)
    cv2.waitKey(1)
    pygame.display.flip()
